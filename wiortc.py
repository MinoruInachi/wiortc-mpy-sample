...
class WioRTC:
    ...
    def begin(self):
        self._change_reg8(WioRTC.PCF8523_ADDRESS,
                          WioRTC.PCF8523_TMR_CLOCKOUT_CTRL,
	...
	if sec <= 255:
	    self._write_reg8(WioRTC.PCF8523_ADDRESS,
                             WioRTC.PCF8523_TMR_B_FREQ_CTRL,
                             0b00000010)    # source for timer B is 1Hz
	    self._write_reg8(WioRTC.PCF8523_ADDRESS,
                             WioRTC.PCF8523_TMR_B_REG,
                             sec)  # timer B value
	elif sec // 60 <= 255:
            self._write_reg8(WioRTC.PCF8523_ADDRESS,
                             WioRTC.PCF8523_TMR_B_FREQ_CTRL,
    ...
    def eeprom_write(self, address, data_buf):
	write_buffer = bytearray(len(data)+2)
	write_buffer[0] = (address >> 8) & 0xff
	write_buffer[1] = address & 0xff
	write_buffer[2:] = data_buf
	self._write(WioRTC.EEPROM_ADDRESS, write_buffer)

    def eeprom_read(self, address, data_buf):
	write_buffer = bytearray(2)
	write_buffer[0] = (address >> 8) & 0xff
	write_buffer[1] = address & 0xff
	self._write(WioRTC.EEPROM_ADDRESS, write_buffer)
	self._read(WioRTC.EEPROM_ADDRESS, data_buf)

    def _write(self, slave_address, data_buf):
        self._wire.writeto(slave_address, data_buf)

    def _read(self, slave_address, data_buf):
        self._wire.readfrom_into(slave_address, data_buf)

    def _write_reg8(self, slave_address, reg, data):
        self._wire.writeto_mem(slave_address, reg, bytearray([data]))

    def _read_reg8(self, slave_address, reg):
        return self._wire.readfrom_mem(slave_address, reg, 1)

    def _change_reg8(self, slave_address, reg, and_val, or_val):
        data_buf = self._read_reg8(slave_address, reg)
        self._write_reg8(slave_address, reg, data_buf[0] & and_val | or_val)
